...
(defun pulseaudio-control--volume (sign)
  (let ((arg (format "set-sink-volume %d %s" (cl-parse-integer (pulseaudio-control--get-default-sink)) (concat sign pulseaudio-control-volume-step))))
    (pulseaudio-control--call-pactl arg)))
